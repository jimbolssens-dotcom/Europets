...
export default function SettleBillOwnerPage() {
  const { clientId } = useParams();
  const [state, setState] = useState('loading'); // loading | due | paid | not_found
  ...
  // (a URL can be typed by hand), so the balance below still comes from
  // our own database, polled until it catches up.
  const [returnedFromNomod, setReturnedFromNomod] = useState(null); // null | 'success' | 'failure'

  const loadBalance = () =>
    fetch(`/api/settle-bill/owner/${clientId}`)
      .then((res) => {
      ...
      .then((data) => {
        setInvoice(data);
        setState(data.balance_due <= 0 ? 'paid' : 'due');
        return data;
      })
    ...
    if (paid === '1') {
      ...
      const interval = setInterval(() => {
        ...
        loadBalance().then((data) => {
          if ((data && data.balance_due <= 0) || attempts >= 12) {
  ...
  async function payNow() {
    setPaying(true);
    setError(null);
    const res = await fetch(`/api/settle-bill/owner/${clientId}`, { method: 'POST' });
    ...
    return (
      <div className="section">
        <div className="container review-submit-narrow">
          ...
          <p className="page-lede">You have no outstanding balance with us. Thank you!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="section">
      <div className="container review-submit-narrow">
        ...
        <p className="page-lede">
          Pay securely online with Nomod — this covers everything currently outstanding on your
          account. No need to come in with cash or a card.
        </p>
