...
export function normalizeClientPhones(rawPhones) {
  ...
  const cleaned = list
    ...
    // A blank row now starts pre-filled with the +971 country code (see
    // ClientPhonesEditor's emptyPhoneRow) rather than truly empty, so a row
    // left untouched has to be dropped here too — otherwise every unused
    // "+ Add" row would get saved as a bare, numberless "+971" entry.
    .filter((p) => p.phone.replace(/\D/g, '').length > 3);
