...
// A cancelled or no-show appointment stays on the schedule/list — it's
// still useful history (the slot was booked, the client didn't show) —
// but shouldn't compete visually with active bookings, so it always
// renders in this flat gray instead of its vet's color.
export const INACTIVE_APPOINTMENT_COLOR = { bg: '#e5e7eb', fg: '#6b7280' };
...
// A surgery (day procedure) appointment gets a darker fill of the same
// vet's own hue instead of a second color to remember — every staff color
// pair is already {bg: light tint, fg: solid hue}, so "darker" is just
// that same pair with the two roles swapped (solid hue as the fill, white
// text) rather than a whole separate dark palette. A consult stays the
// usual light tint. This keeps a vet's consults and surgeries reading as
// "the same color, two shades" on the Appointments schedule.
export function colorForAppointment(vetColorMap, vetId, type) {
  const base = (vetId && vetColorMap[vetId]) || UNASSIGNED_STAFF_COLOR;
  return type === 'surgery' ? { bg: base.fg, fg: '#ffffff' } : base;
}
