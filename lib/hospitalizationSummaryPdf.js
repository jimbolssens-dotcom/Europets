...
export async function buildHospitalizationSummaryPdf({
  ...
  drawText(
    `Patient: ${admission.patients?.name || '—'}${admission.patients?.patient_number ? ` (Patient #${admission.patients.patient_number})` : ''} (${admission.patients?.species || ''})`
  );
  drawText(`Owner: ${admission.clients?.full_name || '—'}${admission.clients?.client_number ? ` (Client #${admission.clients.client_number})` : ''}`);
  drawText(`Room: ${admission.rooms?.name || '—'}`);
  drawText(`Admitted: ${formatDateTime(admission.admitted_at)}`);
  if (admission.discharged_at) {
    drawText(`Discharged: ${formatDateTime(admission.discharged_at)}`);
  }
  ...
  // AI-drafted, client-friendly summary of the whole stay (see
  // generateHospitalizationReport in lib/anthropicClient.js) — drawn
  // first, ahead of the day-to-day worksheet below, since it's the part
  // actually meant for the owner to read. Absent until generated at
  // least once from the Reports section.
  if (admission.ai_summary?.trim()) {
    spacer(8);
    rule();
    spacer(6);
    drawText('Summary for Owner', { size: 12, useFont: bold, color: PINK, gap: 18 });
    for (const paragraph of admission.ai_summary.trim().split(/\n+/)) {
  ...
  for (const group of groupNotesByDate(notes || [])) {
    ...
    for (const n of group.entries) {
      ...
      if (n.weight_kg != null) bits.push(`Weight: ${n.weight_kg} kg`);
  ...
  spacer(10);
  drawText(`Generated ${formatDateTime(new Date())}`, { size: 8, color: GREY, gap: 10 });
