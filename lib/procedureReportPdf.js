...
export async function buildProcedureReportPdf({
  ...
  function drawRichText(text) {
    const rawLines = sanitizeForFont(font, text || '').split(/\n/);
  ...
  drawText(`Owner: ${client?.full_name || '—'}${client?.client_number ? ` (Client #${client.client_number})` : ''}`);
  if (staffName) {
    drawText(`Performed by: ${staffName}`);
  }
  if (performedAt) {
    drawText(`Date: ${formatDateTime(performedAt)}`);
  }
  ...
  if (photos.length) {
    spacer(4);
    drawText('Photos', { size: 12, useFont: bold, color: PINK, gap: 17 });
  ...
  const footerClinicLine = sanitizeForFont(font, [clinicName, TAGLINE].join('  —  '));
