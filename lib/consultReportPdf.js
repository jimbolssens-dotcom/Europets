...
// diagnostics, treatment plan, attached photos) for the vet to send to the
// client — same pdf-lib approach as lib/hospitalizationSummaryPdf.js (pure
// JS, fully embedded standard fonts, no font files read from disk at
// runtime, which avoids a common gotcha bundling font-file-reading PDF
// libraries into Vercel's serverless functions). Header/watermark/footer
// match lib/procedureReportPdf.js (dental/surgical/ultrasound/x-ray
// reports) so every report a client receives looks like it came from the
// same clinic — the image-embedding helpers below are copied from that
...
export async function buildConsultReportPdf({
  visit,
  clinic,
  diagnostics = [],
  treatmentItems = [],
  photos = [], // [{ bytes, contentType, fileName }] — from AttachmentSection uploads
}) {
  ...
  function wrapText(rawText, useFont, size, maxWidth) {
    const text = sanitizeForFont(useFont, rawText);
  ...
  drawText(
    `Patient: ${visit.patients?.name || '—'}${visit.patients?.patient_number ? ` (Patient #${visit.patients.patient_number})` : ''} (${visit.patients?.species || ''})`
  );
  drawText(`Owner: ${visit.clients?.full_name || '—'}${visit.clients?.client_number ? ` (Client #${visit.clients.client_number})` : ''}`);
  ...
  sectionHeading('Exam');
  const hadExamField = [visit.anamnesis, visit.findings, visit.diagnosis, visit.test_results, visit.treatment_notes].some(
    (v) => v
  );
  field('History (owner-reported)', visit.anamnesis);
  field('Findings', visit.findings);
  field('Diagnosis', visit.diagnosis);
  field('Tests', visit.test_results);
  ...
  // Treatment Plan only lists products/services — a test ordered off the
  // catalog creates both a treatment_items row (for billing) and a linked
  // diagnostics row (diagnostics.treatment_item_id) covering it above, so
  // excluding anything category 'test' (or otherwise already linked from
  // a diagnostics row) keeps it from being listed in both sections.
  const diagnosticTreatmentItemIds = new Set(diagnostics.map((d) => d.treatment_item_id).filter(Boolean));
  const treatmentPlanItems = treatmentItems.filter(
    (t) => t.goods_services?.main_category !== 'test' && !diagnosticTreatmentItemIds.has(t.id)
  );

  if (treatmentPlanItems.length) {
    sectionHeading('Treatment Plan');
    for (const t of treatmentPlanItems) {
      newPageIfNeeded(28);
  ...
  if (photos.length) {
    ...
    const images = (await Promise.all(photos.map((p) => embedImage(pdfDoc, p)))).filter(Boolean);
    drawImages(images);
  }
