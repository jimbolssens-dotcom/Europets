...
export async function seedCoreVaccinationsFromLastGiven(supabase, patientId, species, lastVaccinationDate) {
  ...
  try {
    ...
    // The insert always goes through the service-role client, regardless
    // of which `supabase` this function was called with — vaccinations
    // has RLS enabled and its own key can no longer write to it directly.
    await supabaseAdmin.from('vaccinations').insert(rows);
  } catch {
    // Best-effort — a failure here shouldn't take down patient creation.
  }
}

// Precise counterpart to the above for intake submissions that named the
// exact vaccine given (see the portal intake form's species-gated vaccine
// picker) — inserts a single row for that one protocol instead of seeding
// every core protocol for the species blind.
export async function seedVaccinationFromIntake(supabase, patientId, protocolId, dateGiven) {
  if (!protocolId || !dateGiven) return;
  ...
  try {
    ...
    const given = new Date(`${dateGiven}T00:00:00`);
    const due = new Date(given);
    due.setMonth(due.getMonth() + protocol.interval_months);

    // See the same note in seedCoreVaccinationsFromLastGiven above.
    await supabaseAdmin.from('vaccinations').insert([
      {
        patient_id: patientId,
