...
export async function buildConsentFormPdf({ consentForm, patient, client, clinic }) {
  ...
  // wrapped line — used for the form body, which is long-form prose. A
  // single newline within it (e.g. the treatment plan's one-bullet-per-
  // line list — see lib/consentTemplates.js) is kept as its own hard line
  // break rather than word-wrapped away; a blank double-newline paragraph
  // never reaches here (split out by the caller below).
  function paragraph(str, { size = 10.5, useFont = font, color = INK, gap = 14 } = {}) {
    const maxWidth = PAGE_WIDTH - MARGIN * 2;
    for (const rawLine of String(str).split('\n')) {
      for (const l of wrapText(rawLine, useFont, size, maxWidth)) {
        newPageIfNeeded(gap);
  ...
  y -= 16;
  rightText(`Signed: ${formatDateTime(consentForm.signed_at)}`, rightEdge, {
    useFont: font,
    size: 8.5,
    color: GREY,
  });
  ...
  spacer(2);
  line(
    `Patient: ${patient?.name || '—'}${patient?.patient_number ? ` (Patient #${patient.patient_number})` : ''}${patient?.species ? ` (${patient.species})` : ''}`,
    { gap: 14 }
  );
  line(
    `Owner: ${client?.full_name || '—'}${client?.client_number ? ` (Client #${client.client_number})` : ''}`,
    { gap: 14 }
  );
  ...
  line(`Date/time: ${formatDateTime(consentForm.signed_at)}`, { size: 9, color: GREY, gap: 14 });
