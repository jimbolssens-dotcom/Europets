...
const CONSULT_TEXT_FIELDS = ['anamnesis', 'findings', 'diagnosis', 'treatment_notes'];
...
async function resolveRecordingInner(recording) {
  ...
  const isGastroscopyReport = recording.entity_type === 'gastroscopy_report';
  ...
  let summary;
  // Set alongside `summary` only for ultrasound/x-ray/gastroscopy reports —
  // the owner-facing plain-language translation of the scientific report,
  // saved to that report's own client_summary column below.
  ...
  if (!hasSpeech) {
    ...
    const { data: report } = await supabase
      ...
      .select('visits(patients(name, species)), hospitalizations(patients(name, species))')
      .eq('id', recording.entity_id)
      .single();
    const patient = report?.visits?.patients || report?.hospitalizations?.patients;
    ...
    clientSummary = await generateClientSummaryFromScanReport({
    ...
    summary = await generateGastroscopyReport({
      transcript,
      patientName: patient?.name,
      species: patient?.species,
    });
    ...
    [{ data: tests }, { data: productsAndServices }] = await Promise.all([
      ...
      supabase
        .from('goods_services')
        .select('id, name, administration_method')
  ...
  if (isVisit && hasSpeech) {
    ...
    if (Object.keys(update).length > 0) {
      await supabaseAdmin.from('visits').update(update).eq('id', recording.entity_id);
      if (update.weight_kg !== undefined && visit?.patient_id) {
        await supabaseAdmin.from('patients').update({ current_weight_kg: update.weight_kg }).eq('id', visit.patient_id);
      }
    }
    ...
    if (fields.diagnostics_ordered?.length) {
      for (const name of fields.diagnostics_ordered) {
        ...
        const { data: treatmentItem, error: itemError } = await supabaseAdmin
    ...
    if (fields.treatments_given?.length) {
      for (const t of fields.treatments_given) {
        ...
        await supabaseAdmin.from('treatment_items').insert([
          {
            visit_id: recording.entity_id,
            goods_service_id: match.id,
            instructions: t.instructions || null,
            quantity: t.quantity || 1,
            // Copied from the catalog item's own fixed classification, same
            // as the manual "add from catalog" path (POST /api/treatment-
            // items) — a dictated medication was silently missing this,
            // which meant it never picked up its dispensing/SC/IM fee once
            // the treatment plan was invoiced (see lib/invoicing.js).
            administration_method: resolveAdministrationMethod(match.administration_method).administration_method,
          },
        ]);
      }
    }
    ...
    // way to recover it.
    const { error: surgicalReportError } = await supabaseAdmin
    ...
    const { error: dentalReportError } = await supabaseAdmin
      .from('dental_reports')
      .update({ ai_summary: summary })
      .eq('id', recording.entity_id);
    ...
    const { error: ultrasoundReportError } = await supabaseAdmin
      .from('ultrasound_reports')
      .update({ ai_summary: summary, client_summary: clientSummary })
      .eq('id', recording.entity_id);
    ...
    const { error: xrayReportError } = await supabaseAdmin
      .from('xray_reports')
      .update({ ai_summary: summary, client_summary: clientSummary })
      .eq('id', recording.entity_id);
    if (xrayReportError) {
  ...
  } else if (isGastroscopyReport && hasSpeech) {
    const { error: gastroscopyReportError } = await supabaseAdmin
      .from('gastroscopy_reports')
      .update({ ai_summary: summary, client_summary: clientSummary })
      .eq('id', recording.entity_id);
    if (gastroscopyReportError) {
      throw new Error(`Failed to save gastroscopy report summary: ${gastroscopyReportError.message}`);
    }
    ...
    for (const task of fields || []) {
      ...
      const { error: insertError } = await supabaseAdmin.from('hospitalization_plan_items').insert([
        {
          hospitalization_id: recording.entity_id,
          label: task.label,
          goods_service_id: match?.id || null,
          instructions: task.instructions || null,
          is_surgical: !!task.is_surgical,
        },
      ]);
