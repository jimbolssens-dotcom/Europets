...
// the Accounting > Unpaid Invoices page. Reminders are whatsapp://mailto:
// deep links, same as everywhere else messaging happens in this app:
// nothing is sent server-side, no delivery tracking, staff sends it
// themselves.
//
// Each reminder links straight to that invoice's own "Settle Your Bill"
// page (website/app/settle-bill/[id]) — the same link the invoice page's
// own "Send Payment Link" button uses — so the client can pay online
// without a second message.

import { openWhatsApp } from './whatsapp';

function websiteUrl() {
  return process.env.NEXT_PUBLIC_WEBSITE_URL || 'https://epc.vet';
}

function settleBillUrl(inv) {
  return `${websiteUrl()}/settle-bill/${inv.id}`;
}
...
export function reminderBody(clientName, invoices) {
  ...
  if (invoices.length === 1) {
    ...
    return `Hi ${name}, this is a friendly reminder from Europets Clinic that ${singleInvoiceLine(inv)} is still outstanding. You can pay securely online here: ${settleBillUrl(inv)}\n\nPlease let us know if you have any questions. Thank you!`;
  }
  const lines = invoices
    .map((inv) => `${invoiceLabel(inv)} (AED ${money(balanceDue(inv))}) — pay here: ${settleBillUrl(inv)}`)
    .join('\n');
  return `Hi ${name}, this is a friendly reminder from Europets Clinic that you have a total outstanding balance of AED ${money(totalBalanceDue(invoices))} across ${invoices.length} invoices:\n${lines}\n\nPlease let us know if you have any questions. Thank you!`;
}
...
// the last line of defense against opening a broken whatsapp:/mailto: link.
export function openWhatsAppReminder(phone, clientName, invoices) {
  return openWhatsApp(phone, reminderBody(clientName, invoices));
}
