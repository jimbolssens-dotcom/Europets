...
import { createDischargeFollowupIfEligible } from '@/lib/dischargeFollowups';
...
export async function runHospitalizationDischargeEffects(supabase, hospitalizationId) {
  ...
  // Queues a "how's recovery going?" WhatsApp check-in for a surgical/
  // dental stay — a no-op for a plain boarding/observation admission (see
  // lib/dischargeFollowups.js). Review-first: this only ever creates the
  // scheduled row, never sends anything itself. Best-effort, same
  // reasoning as the two blocks above — the discharge itself must never
  // fail because of this.
  try {
    await createDischargeFollowupIfEligible(hospitalizationId);
  } catch (err) {
    console.error('Failed to schedule a discharge follow-up', hospitalizationId, err);
  }
}
