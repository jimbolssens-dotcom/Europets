...
const CATEGORY_BG = rgb(0.961, 0.961, 0.961); // #f5f5f5, matches .invoice-category-row on-screen

const TAGLINE = 'Kind, caring, and compassionate veterinary care';

// Groups line items into their own labeled sections on the invoice —
// Services first, then Diagnostics (catalog main_category 'test'), then
// Medications (main_category 'product') — same idea as the on-screen
// admin invoice's category rows (lib/catalogGrouping.js), but its own
// order/labels since a client-facing tax invoice reads better with the
// clinical work up front. Items whose catalog link is missing fall into
// a trailing "Other" section rather than being dropped.
...
function groupLineItemsForPdf(lineItems) {
  ...
  return ordered.map((mc) => ({
    mainCategory: mc,
    label: mc ? PDF_CATEGORY_LABELS[mc] : 'Other',
...
export async function buildTaxInvoicePdf({ invoice, lineItems, clinic, client, patient }) {
  ...
  y -= 12;
  if (invoice.paid_at) {
    rightText(`Paid date: ${new Date(invoice.paid_at).toLocaleDateString('en-AE')}`, rightEdge, {
      useFont: font,
      size: 9,
      color: GREY,
    });
  ...
  const statusLabel = invoice.status === 'partially_paid' ? 'Partially Paid' : invoice.status;
  rightText(`Status: ${statusLabel}`, rightEdge, { useFont: font, size: 9, color: GREY });
  // Captured before the left column below resets y — the right column's
  // line count varies (paid date only when invoice.paid_at is set), so its
  // own bottom edge has to be included in the header's final height too,
  // or a shorter left column (a clinic with little contact info filled in)
  // lets the Bill To section below start too high and overlap it.
  ...
  const groups = groupLineItemsForPdf(lineItems);
  for (const group of groups) {
    ...
    for (const item of group.items) {
      ...
      // Qty here is the billed total (per-dose quantity × times given — see
      // migrations/129) so it still multiplies out to the printed Total, the
      // same as before that split existed; times_given on its own is an
      // internal editing detail (see the invoice detail page), not something
      // a client reading this PDF needs broken out separately.
      const billedQty = Math.round(Number(item.quantity) * (Number(item.times_given) || 1) * 100) / 100;
      text(String(billedQty), cols.qty, { size: 10 });
  ...
  newPageIfNeeded(discountAmount > 0 ? 71 : 56);
  text('Subtotal (excl. VAT):', cols.vat - 90, { size: 10, color: GREY });
  text(`AED ${money(invoice.subtotal)}`, cols.total, { size: 10 });
  y -= 15;
  if (discountAmount > 0) {
    text('Discount:', cols.vat - 90, { size: 10, color: GREY });
    text(`-AED ${money(discountAmount)}`, cols.total, { size: 10 });
    y -= 15;
  }
  ...
  if (amountPaid > 0) {
    y -= 15;
    text('Amount Paid:', cols.vat - 90, { size: 10, color: GREY });
    text(`AED ${money(amountPaid)}`, cols.total, { size: 10 });
    y -= 15;
    const balanceDue = Math.max(0, Number(invoice.total) - amountPaid);
    text('Balance Due:', cols.vat - 90, { size: 11, useFont: bold });
    text(`AED ${money(balanceDue)}`, cols.total, { size: 11, useFont: bold });
  }
