...
// Actually ends a call — deletes the room outright rather than just
// marking it ended in our own database, so the link staff sent the client
// truly stops working (ejects anyone still in it, and Daily 404s on the
// room afterward) instead of leaving it silently joinable until its exp.
export async function deleteDailyRoom(roomName) {
  ...
  // Already gone (e.g. it expired on its own, or was already ended) is a
  // fine outcome here, not a failure — this is meant to make sure a room
  // is dead, not to prove it existed a moment ago.
  if (!res.ok && res.status !== 404) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.error || data.info || `Failed to end the call (HTTP ${res.status})`);
  }
}
