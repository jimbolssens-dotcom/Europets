...
export async function runConsultCompletionEffects(supabase, visit) {
  if (visit.appointment_id) {
    await supabase.from('appointments').update({ status: 'complete' }).eq('id', visit.appointment_id);
  }
  ...
  try {
    ...
    if (clientReport) {
      await supabase.from('visits').update({ ai_summary: clientReport }).eq('id', visit.id);
