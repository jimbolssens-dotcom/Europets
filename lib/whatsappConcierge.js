...
// checking/booking a ROUTINE CONSULT, SPAY, CASTRATION, or DENTAL
// (cleaning or extractions) for one of that client's existing pets. Called
// once per inbound WhatsApp text message — see maybeRunConcierge, the only
// export the webhook calls.
//
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      'Hand this conversation to a human member of staff instead of replying yourself. No message is sent to the client — they just see it in the normal inbox like any other message. Always use this for anything medical or symptom-related, anything about money already paid, a video consult or any non-standard/custom surgery request, cancelling or rescheduling an existing appointment, or anything at all you are not fully confident about.',
    input_schema: {
      type: 'object',
      properties: {
...
2. Check availability for, and book, a ROUTINE CONSULT (always exactly 15 minutes) for one of the pets listed above, using check_consult_availability/book_consult.
...
- the client wants a video consult, or a surgery that isn't a standard spay/castration/dental (an emergency, or anything you can't cleanly fit to one of your procedure types)
