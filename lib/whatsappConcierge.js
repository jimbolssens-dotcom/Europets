...
export async function maybeRunConcierge({ clientId, phone, dryRun = false, replayLastClientMessage = false }) {
  ...
  try {
    ...
    // forth testing, the newest HISTORY_LIMIT rows can be entirely staff/
    // ai (see below), which would otherwise empty the window out before
    // ever reaching a client message.
    const fetchLimit = replayLastClientMessage ? HISTORY_LIMIT * 3 : HISTORY_LIMIT;
