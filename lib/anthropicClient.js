...
import { DIAGNOSTIC_ABNORMALITIES_FROM_DOCUMENT_INSTRUCTIONS } from './diagnosticReportPolicy';
...
// Same idea as generateUltrasoundReport/generateXrayReport above, for an
// endoscopic exam of the upper GI tract — the vet dictates what they see
// scope-in (esophagus, stomach, duodenum, mucosal appearance, any foreign
// body or mass, whether biopsies were taken) and this elaborates it into
// the formal gastroscopy report for the medical record, using standard
// veterinary endoscopy terminology. Also not client-facing — see
// generateClientSummaryFromScanReport below for that pass. No post-op
// half here either — a diagnostic scope has no home-care of its own.
export async function generateGastroscopyReport({ transcript, patientName, species }) {
  const message = await anthropic.messages.create({
    model: 'claude-opus-5',
    max_tokens: 1024,
    system: `You are a veterinarian writing the formal gastroscopy (upper GI endoscopy) report for a patient's permanent medical record, based on the attending veterinarian's findings below (dictated or typed). This is a clinical document for the record and for referring/consulting veterinarians — NOT for the pet owner — so write in precise, professional veterinary terminology throughout, grounded strictly in what was actually stated; never invent a finding, region, or measurement not mentioned. Structure the report with these labeled sections: FINDINGS — a systematic account of what was examined (esophagus, stomach, pylorus, duodenum, as applicable), using standard endoscopic descriptors exactly where the findings support them (e.g. mucosal erythema/erosion/ulceration, friability, hemorrhage, mass, stricture, foreign body, and whether biopsies were taken and from where) — and IMPRESSION — a concise clinical summary of the significant findings and their likely significance, staying within what the findings actually support. Plain section labels in capitals, no markdown "#" headings. Return only the finished report, nothing else.`,
    messages: [
      {
        role: 'user',
        content: `Patient: ${patientName || 'the patient'} (${species || 'unknown species'})\n\nThe vet's gastroscopy findings (dictated or typed):\n"""\n${transcript.trim()}\n"""`,
      },
    ],
    output_config: { effort: 'medium' },
  });
...
// The owner-facing translation of an ultrasound/x-ray/gastroscopy report
// generated above — reads the finished clinical report (not the raw
// dictation) and renders it in plain, warm language a worried owner can
// follow, so the client never sees raw veterinary terminology. Grounded
// only in what the clinical report actually says; nothing invented beyond
// it. Already generic across scan kinds (scanKind is just interpolated
// into the prompt), so no gastroscopy-specific branch is needed here.
...
// Only ever called from a staff-pressed "AI interpretation" button (see
// RecordReports.jsx) — never automatically on upload — and deliberately
// abnormalities-only: no normal values, no patient details, no client
// details, just a compact list of what's actually out of range or
// positive. Free text rather than JSON — result formats vary too widely
// (a values table, a reader's typed summary, a scanned printout) to force
// into a fixed schema.
