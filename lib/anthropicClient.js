...
const ConsultFieldsSchema = z.object({
  ...
  diagnostics_ordered: z
    .array(z.string())
    .describe(
      'Short names of diagnostic tests actually ordered or run during this consult (e.g. "CBC", "X-ray of the left front leg", "urinalysis", "skin scrape") — one short catalog-style name per test, not a sentence. Only include tests actually ordered/run, not ones merely discussed as an option. Empty array if none.'
    ),
  treatments_given: z
    .array(
      z.object({
        name: z
          .string()
          .describe('Short catalog-style name of the medication, product, or service (e.g. "Optidex", "Cerenia injection"), not a sentence.'),
        instructions: z
          .string()
          .nullable()
...
// Vitals & Exam form (visits.anamnesis/findings/diagnosis/prognosis/
// treatment_notes), instead of one freeform note, plus the diagnostic
// tests and treatments actually given — the webhook that calls this then
// matches those against the goods_services catalog and adds them to the
// Diagnostics/Treatment Plan lists directly.
