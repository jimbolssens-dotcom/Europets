...
const DENTAL_SYSTEM_PROMPT = `You are a veterinary scribe. You are given a raw speech-to-text transcript of a vet narrating during or after a dental procedure. Turn it into a concise dental note.

Structure it with short headings only where relevant to what was actually said (skip any that don't apply): Findings, Procedures performed, Extractions, Complications, Post-op plan. Use plain clinical language, third person, no filler. Do not invent details that weren't mentioned.

If a "current dental chart" note is provided below, treat it as the source of truth for which teeth are extracted or already missing, and fold it into Extractions/Findings (by tooth label, e.g. "P4 (upper right)") even if the vet didn't call out every tooth by name in the recording — don't just repeat the chart note verbatim, write it as part of the clinical note.`;
...
// Drafts owner-facing post-op care instructions for one specific patient's
// surgical/dental report. The clinic's standard baseline (edited/approved
// on the Settings page) is the starting point and standard of care —
// adapted to what this particular case's notes/AI summary actually say
// (medications given, findings, anything unusual), not handed back
// verbatim. Nothing from here reaches an owner automatically: the vet
// reviews and edits the draft, then explicitly saves it, before it can be
// downloaded/shared (see app/(admin)/consults/[id]/page.jsx).
export async function generatePostOpInstructions({ procedureType, patientName, species, caseNotes, baseline }) {
  const procedureLabel = POSTOP_PROCEDURE_LABELS[procedureType] || 'procedure';

  const baselineSection = baseline?.trim()
    ? `The clinic's standard post-op instructions for a ${procedureLabel} are:\n"""\n${baseline.trim()}\n"""\nUse these as your starting point and standard of care — keep everything in them that still applies, and only depart from them where this specific case's notes below give a clear reason to (e.g. an extra medication given, a complication, a precaution the vet mentioned).`
    : `The clinic has no standard baseline instructions on file for a ${procedureLabel} yet — draft sensible, conservative general post-op care instructions appropriate for this kind of procedure in a companion animal.`;

  const notesSection = caseNotes?.trim()
    ? `This specific case's clinical notes:\n"""\n${caseNotes.trim()}\n"""`
    : `No clinical notes were recorded for this specific case yet — draft from the baseline alone.`;

  const message = await anthropic.messages.create({
    model: 'claude-opus-5',
    max_tokens: 1024,
    system: `You are a veterinary assistant drafting post-operative/post-procedure home-care instructions to hand or send to a pet owner after a ${procedureLabel}. Write directly to the owner, second person ("your pet" / the pet's name), warm but clear plain language — no clinical jargon, no markdown "#" headings, just short labeled sections or bullet points a worried owner can skim (e.g. Activity, Wound/mouth care, Feeding, Medications, When to call us). Do not invent specifics (medication names/doses, findings) that aren't supported by the case notes or the baseline. Keep it concise — this goes on a printed handout. Return only the finished instructions, nothing else.`,
    messages: [
      {
        role: 'user',
