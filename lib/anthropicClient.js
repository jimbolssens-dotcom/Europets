...
// Turns a completed consult's own clinical notes (already written by the
// vet, by hand or via the recording pipeline's summarizeTranscript/
// extractConsultFields above) into a client-facing report — what was
// found and done today, in plain language, plus any home-care advice
// implied by the treatment notes. Unlike generateClientReport (surgical/
// dental), there's no separate dictation transcript at this point — the
// visit's own text fields are the source material, since a regular
// consult only gets summarized once, at completion. Returns '' if there
// were no clinical notes recorded worth writing up.
export async function generateConsultReport({ patientName, species, anamnesis, findings, diagnosis, prognosis, treatmentNotes }) {
  const clinicalNotes = [
    anamnesis?.trim() && `Reason for visit / history:\n${anamnesis.trim()}`,
    findings?.trim() && `Exam findings:\n${findings.trim()}`,
    diagnosis?.trim() && `Diagnosis:\n${diagnosis.trim()}`,
    prognosis?.trim() && `Prognosis:\n${prognosis.trim()}`,
  ...
  const message = await anthropic.messages.create({
    model: 'claude-opus-5',
    max_tokens: 1024,
    system: `You are a vet writing a short report for a pet owner, summarizing today's consult in plain, warm language — no clinical jargon a non-vet wouldn't know. Cover what was found, what was done, and any home-care instructions or follow-up implied by the notes (medications given, what to watch for, when to come back) — but only what's actually supported by the notes below; never invent advice that isn't grounded in them. Keep it concise — a few short paragraphs, not a wall of text. Never soften or omit a concerning finding.`,
    messages: [
      {
        role: 'user',
...
// The client-facing report for an ultrasound scan: the vet dictates raw,
// terse findings (organ by organ, whatever they call out) and this
// elaborates them into a coherent report a worried owner can follow, plus
// a brief overall impression — grounded only in what was actually said,
...
// webhook), same as the other procedure reports; the vet can still edit
// the saved result before it's shared.
export async function generateUltrasoundReport({ transcript, patientName, species }) {
  const message = await anthropic.messages.create({
    model: 'claude-opus-5',
    max_tokens: 1024,
    system: `You are a veterinary assistant writing an ultrasound report for a pet owner, based on the vet's dictated findings below. Elaborate clearly on what was found — organ by organ if the vet went through them — in warm, plain language a worried owner can follow, without inventing anything not said in the dictation. End with a brief overall impression. Write directly to the owner, second person ("your pet" / the pet's name). No clinical jargon, no markdown "#" headings. Return only the finished report, nothing else.`,
    messages: [
      {
        role: 'user',
        content: `Patient: ${patientName || 'the patient'} (${species || 'unknown species'})\n\nTranscript of the vet's dictated ultrasound findings:\n"""\n${transcript.trim()}\n"""`,
      },
    ],
    output_config: { effort: 'medium' },
  });
...
export async function generateXrayReport({ transcript, patientName, species }) {
  const message = await anthropic.messages.create({
    model: 'claude-opus-5',
    max_tokens: 1024,
    system: `You are a veterinary assistant writing an x-ray (radiograph) report for a pet owner, based on the vet's dictated findings below. Elaborate clearly on what was seen on the x-ray in warm, plain language a worried owner can follow, without inventing anything not said in the dictation. End with a brief overall impression. Write directly to the owner, second person ("your pet" / the pet's name). No clinical jargon, no markdown "#" headings. Return only the finished report, nothing else.`,
    messages: [
      {
        role: 'user',
        content: `Patient: ${patientName || 'the patient'} (${species || 'unknown species'})\n\nTranscript of the vet's dictated x-ray findings:\n"""\n${transcript.trim()}\n"""`,
      },
    ],
    output_config: { effort: 'medium' },
  });
    ...
    messages: [
      {
        ...
        content: [
          ...
          },
          {
            ...
            } — a lab panel, imaging reading, or similar. Transcribe the result values/findings into a concise plain-text summary suitable for a patient's clinical record (e.g. "WBC 12.4 (H), RBC 6.1, ..." for a lab panel, or a short findings paragraph for imaging). Do not add commentary, interpretation, or anything not shown in the photo. If the photo isn't legible or doesn't look like a test result, respond with exactly: UNREADABLE`,
          },
        ],
      },
    ],
