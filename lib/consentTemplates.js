...
function formatTreatmentPlanSection({ treatmentNotes, treatmentItems } = {}) {
  ...
  const items = (treatmentItems || []).filter((item) => item.goods_services?.name || item.instructions);
  if (!notes && items.length === 0) return null;

  const itemLines = items
    .map((item) => {
      const name = item.goods_services?.name || 'Item';
...
export function buildConsentFormText(formType, patient = {}, treatmentPlan = {}) {
  ...
  return [buildBody({ ...patient, patientName }), planSection, LIABILITY_CLAUSE, SIGNATURE_NOTE]
    .filter(Boolean)
    .join('\n\n');
}
