...
// microchip, a nail trim...), but the two that are actual surgical/dental
// procedures — a spay/neuter or a dental — need their own specific
// consent language folded in (the tattoo clause, the anesthesia/
// extraction risk, etc.), not just the generic day-patient paragraph
// below. Reuses the same body text as the standalone surgery_standard_
// neuter/dental forms verbatim, so there's one place that wording lives.
// A complex/advanced surgery is deliberately NOT checked for here — that
// always goes through a consult and gets its own surgery_complex form,
// never a day procedure.
