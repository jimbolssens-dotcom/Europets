...
export async function ensureHospitalizationCharges(supabase, hospitalizationId, { dogSize } = {}) {
  const { data: hosp, error: hospError } = await supabase
    ...
    .select(
      'id, kind, status, admitted_at, discharged_at, originating_visit_id, hospitalization_rate_override_id, patients(species, current_weight_kg)'
    )
  ...
  // Staff sometimes add an upfront estimate straight onto the originating
  // consult's own treatment plan (e.g. "roughly a week's stay") before the
  // admission's day-to-day worksheet exists yet. This function's own
  // dedup below only ever looks at treatment_items tied to THIS
  // hospitalization's own notes, so it has no way to see that estimate —
  // left unchecked, it backfills the real day-by-day rate on top of it,
  // double-billing (this is exactly what happened to a live case: a
  // 7-day consult-side estimate plus an 11-day real accrual on the same
  // stay, landing as two separate, undetected line items on one invoice).
  // Once ANY hospitalization-rate item exists on the consult, trust that
  // staff are billing this stay's rate manually and never auto-accrue for
  // it — safer to stop than to risk charging the same days twice.
  if (hosp.originating_visit_id && rateCatalogIds.length > 0) {
    const { data: visitRateItems, error: visitRateError } = await supabase
      .from('treatment_items')
      .select('id')
      .eq('visit_id', hosp.originating_visit_id)
      .in('goods_service_id', rateCatalogIds)
      .limit(1);
    if (visitRateError) return { error: visitRateError };
    if (visitRateItems && visitRateItems.length > 0) return { skipped: true };
  }
