...
//            the case — nothing else clears it automatically; a rapid
//            weight loss (see lib/weightLossAlarm.js — 5%+ over ~3 days,
//            computed live from the worksheet's own weight readings) also
//            renders red for the same urgency, but — unlike a checkup
//            request — clears itself the moment the underlying trend no
//            longer holds (weight recovers, or the 3-day window moves
//            past the drop), with no explicit "mark checked" step
...
export function hospitalizationAttentionReasons(hosp) {
  ...
  if (hosp.weight_loss_alarm) {
    const { fromWeight, toWeight, percent, days } = hosp.weight_loss_alarm;
    red.push(`Lost ${percent}% of body weight over ${days} days (${fromWeight}kg → ${toWeight}kg)`);
  }
