...
function confirmationMessage(clientName, patientName, startTimeISO) {
  ...
  const dateLabel = d.toLocaleDateString('en-GB', { weekday: 'long', day: '2-digit', month: '2-digit' });
...
export function useIntakeReview(reload) {
  ...
  async function review(id, action, clientId, request, clientName) {
    ...
    if (action === 'approve' && request?.appointment_type) {
      // A video consult has no room to pick — see lib/appointmentScheduling.js's findAppointmentConflict.
      if (!approvalRoom[id] && request.appointment_type !== 'video') {
    ...
    try {
      ...
      // The server already sends the confirmation itself, from the
      // clinic's own WhatsApp Business number (see POST
      // /api/intake-requests/:id's sendBookingConfirmation call) — this is
      // only the fallback for when that failed (template not approved
      // yet, send error, etc.): one click opens WhatsApp pre-filled with
      // the confirmed time on staff's own personal WhatsApp instead, same
      // as this used to work for every approval.
      if (action === 'approve' && data?.appointment && data.clients?.phone && !data.confirmation_sent) {
        openWhatsApp(
          data.clients.phone,
