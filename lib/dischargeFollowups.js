...
function buildFollowupMessage(row) {
  ...
  return vetName
    ? `${vetName} wanted to check in — how's the recovery going? Reply here anytime if you have questions or concerns.`
    : "How's the recovery going? Reply here anytime if you have questions or concerns.";
}
...
export async function sendDischargeFollowup(row, message, sentBy) {
  ...
  const fullMessage = `Hi ${clientFirst}, following up on ${patientName}'s ${procedure}: ${message}`;
