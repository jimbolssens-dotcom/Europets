...
export async function buildProformaQuotePdf({ quote, items, clinic, client, patient }) {
  ...
  newPageIfNeeded(56);
  rightText('Subtotal (excl. VAT):', cols.total - 10, { size: 10, useFont: font, color: GREY });
  text(`AED ${money(subtotal)}`, cols.total, { size: 10 });
  y -= 15;
  rightText(`VAT (${vatPercent}):`, cols.total - 10, { size: 10, useFont: font, color: GREY });
  text(`AED ${money(vatAmount)}`, cols.total, { size: 10 });
  y -= 15;
  rightText('Estimated Total (incl. VAT):', cols.total - 10, { size: 12, useFont: bold });
