...
// The placeholder above makes every admin write fail with Supabase's own
// generic "Invalid API key" — surface *why* in the function logs instead
// of leaving that as the only signal, since it's otherwise indistinguishable
// from a genuinely wrong key value.
if (serviceRoleKey === 'placeholder-service-role-key') {
  console.error(
    'supabaseAdmin: SUPABASE_APP_SERVICE_ROLE_KEY is not set in this environment — every admin write will fail with "Invalid API key".'
  );
}
