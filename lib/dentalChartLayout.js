...
// Spelled out for describeDentalChart below — a bare numeric label like
// "104" carries no hint of which tooth type that actually is (dogs and
// cats both use the same quadrant+position digits here), and the AI
// drafting the client report has no other way to know a canine was
// pulled instead of, say, an incisor.
const TOOTH_TYPE_NAMES = {
  I: 'incisor',
  C: 'canine',
  P: 'premolar',
  M: 'molar',
...
function upperTooth(id, label, type, side, t) {
  ...
  return { id, label, type, arch: 'upper', side: side === -1 ? 'left' : 'right', cx: x, cy: y, ...SIZE[type] };
}
function lowerTooth(id, label, type, side, t) {
  const { x, y } = archXY(side, t, LOWER_APEX_Y, -1);
  return { id, label, type, arch: 'lower', side: side === -1 ? 'left' : 'right', cx: x, cy: y, ...SIZE[type] };
}
...
export function describeDentalChart(species, chart) {
  ...
  for (const tooth of layout) {
    ...
    const typeName = TOOTH_TYPE_NAMES[tooth.type] || tooth.type;
    const desc = `${tooth.label} — ${typeName} (${tooth.arch} ${tooth.side})`;
