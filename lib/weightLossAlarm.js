...
//
// Acknowledgeable, unlike the rest of this app's purely computed alarms —
// staff can mark one attended-to (hospitalizations.weight_loss_ack_reading_at,
// migration 144), which suppresses it as long as the reading it's based on
// stays the most recent one. The moment a newer weight is logged, that's a
// new data point nobody's seen yet, so the alarm re-evaluates fresh against
// it rather than staying silenced indefinitely.
...
// alarm rather than a false one off too short a window. ackReadingAt (ISO
// string or Date, optional) suppresses the result if the current reading
// is no newer than it — i.e. staff already acknowledged this exact reading.
export function detectWeightLossAlarm(readings, ackReadingAt) {
  ...
  if (ackReadingAt && current.created_at <= new Date(ackReadingAt)) return null;
  ...
  return {
    ...
    readingAt: current.created_at.toISOString(),
  };
}
