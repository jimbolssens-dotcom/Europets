...
// isXrayReport/isGastroscopyReport branches) — but from a report row's own
// manually typed fields instead of a recording transcript, for a surgical/
// dental/ultrasound/x-ray/gastroscopy report added by hand rather than
// dictated. Called by each report type's own POST .../generate-report
// route.
...
const REPORT_TABLES = {
  ...
  gastroscopy_report: 'gastroscopy_reports',
};
...
// generateClientReport/generateUltrasoundReport/generateXrayReport/
// generateGastroscopyReport below unchanged — those don't care whether the
// text came from a transcript or was typed by hand.
function manualEntryText(reportType, row) {
  ...
  // ultrasound_report / xray_report / gastroscopy_report
...
export async function generateReportFromManualEntry(reportType, reportId) {
  ...
  } else if (reportType === 'gastroscopy_report') {
    summary = await generateGastroscopyReport({ transcript: text, patientName: patient?.name, species: patient?.species });
    clientSummary = await generateClientSummaryFromScanReport({ clinicalReport: summary, patientName: patient?.name, scanKind: 'gastroscopy' });
  } else {
  ...
  // supabaseAdmin, not the public client used for the read above: these
  // report tables only have a public-read RLS policy (see
  // migrations/103_rls_lockdown.sql), no write policy, so an update
  // through the public client silently touches zero rows — indistinguishable
  // from "the row was deleted" below, which is exactly the bug this used to
  // have (every manual "Generate report" click failed with that message,
  // no dictation/generation was actually lost).
