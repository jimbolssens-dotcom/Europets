...
export function formatConsultReportSources({ diagnostics = [], dental = [], surgical = [], ultrasound = [], xray = [], gastroscopy = [] }) {
  const entries = diagnostics.map((row) => ({
    label: row.goods_services?.name || row.type || 'Test',
    text: row.result || 'Result not yet recorded.',
  }));
  for (const [label, rows] of [['Dental', dental], ['Surgery', surgical], ['Ultrasound', ultrasound], ['X-ray', xray], ['Gastroscopy', gastroscopy]]) {
