...
// sufficient — the phone number itself also has to be explicitly told to
// route its events to this app, via this call. Skip it and every dashboard
// setting can show green while Meta silently never calls the webhook for
// real messages. Safe to call again later (e.g. after rotating the access
// token, or if the number is ever re-registered) — it's idempotent.
export async function subscribeWhatsAppWebhook() {
  ...
  const res = await fetch(`https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/subscribed_apps`, {
    method: 'POST',
