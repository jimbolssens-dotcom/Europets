...
// template. v2 (see submitClientAppLinkTemplate below): Meta silently
// recategorized the original client_app_link template as MARKETING during
// review — its generic "come check out the app" pitch with zero
// personalization read as promotional rather than transactional, which
// subjects it to Meta's per-recipient marketing-message send limits
// (error #131049, "to maintain healthy ecosystem engagement") regardless
// of approval status or the sending number's own quality rating. This
// version leads with the client's own name instead, matching the pattern
// of every template here that stayed UTILITY.
export async function sendClientAppLinkMessage(phoneDigits, { clientName } = {}) {
  const accessToken = process.env.META_WHATSAPP_ACCESS_TOKEN;
  const phoneNumberId = process.env.META_WHATSAPP_PHONE_NUMBER_ID;
  const templateName = process.env.META_WHATSAPP_CLIENT_APP_TEMPLATE_NAME || 'client_app_link_v2';
  ...
  const res = await fetch(`https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/messages`, {
    ...
    body: JSON.stringify({
      ...
      template: {
        ...
        components: [{ type: 'body', parameters: [{ type: 'text', text: clientName || 'there' }] }],
      },
    }),
  });
...
// One-time setup: proposes the client_app_link_v2 template to Meta for
// review — see submitConsentFormTemplate above for the pattern. A new
// template name, not an edit to the original client_app_link — Meta
// doesn't let an existing template's category be changed after the fact,
// only a fresh submission gets re-classified.
  ...
  const res = await fetch(`https://graph.facebook.com/${GRAPH_VERSION}/${wabaId}/message_templates`, {
    ...
    body: JSON.stringify({
      ...
      components: [
        {
          type: 'BODY',
          text: "Hi {{1}}, you can view your pets' appointments, invoices, and records anytime in the Europets Client App.",
          example: { body_text: [['Jim']] },
        },
        {
          type: 'BUTTONS',
...
// lib/dischargeFollowups.js). v2 (see submitDischargeFollowupTemplate
// below): the original discharge_followup_checkin template got silently
// recategorized as MARKETING during review — leading with "checking in"
// and burying the patient/procedure inside a free-text variable read as
// soft/promotional rather than transactional (error #131049, "to
// maintain healthy ecosystem engagement"). This version puts the patient
// and procedure directly in the template's fixed structure instead, only
// leaving the closing line (message) as the staff-editable part.
export async function sendDischargeFollowupMessage(phoneDigits, { clientName, patientName, procedure, message }) {
  const accessToken = process.env.META_WHATSAPP_ACCESS_TOKEN;
  const phoneNumberId = process.env.META_WHATSAPP_PHONE_NUMBER_ID;
  const templateName = process.env.META_WHATSAPP_DISCHARGE_FOLLOWUP_TEMPLATE_NAME || 'discharge_followup_checkin_v2';
  ...
  const components = [
    {
      ...
      parameters: [
        { type: 'text', text: clientName || 'there' },
        { type: 'text', text: patientName || 'your pet' },
        { type: 'text', text: procedure || 'their recent procedure' },
...
// One-time setup: proposes the discharge_followup_checkin_v2 template to
// Meta for review — see submitFirstContactTemplate above for the pattern.
// A new template name, not an edit to the original — Meta doesn't let an
// existing template's category be changed after the fact.
  ...
  const res = await fetch(`https://graph.facebook.com/${GRAPH_VERSION}/${wabaId}/message_templates`, {
    ...
    body: JSON.stringify({
      ...
      components: [
        {
          type: 'BODY',
          // Meta rejected the first attempt at this ("too many variables
          // for its length" — 4 variables against a body that was mostly
          // just those variables) — this version keeps the same 4 but
          // pads the fixed text around them so the ratio passes, and moves
          // the "reply anytime" closer into the fixed portion so {{4}}
          // itself can stay short (see buildFollowupMessage's shorter
          // default tail in lib/dischargeFollowups.js).
          text: "Hi {{1}}, this is Europets Veterinary Clinic following up on {{2}}'s {{3}}. {{4}} Feel free to reply here anytime if you have questions or concerns.",
          example: {
            body_text: [['Jim', 'Bruce', 'dental cleaning', "How's the recovery going?"]],
          },
        },
      ],
    }),
...
// Diagnostic for "a message failed with a vague ecosystem/engagement-sounding
// reason" (see the "⚠️ Not delivered" text on a client thread) — pulls this
// number's live standing straight from Meta rather than guessing from the
// error text alone. quality_rating is GREEN/YELLOW/RED (RED means Meta is
// actively restricting sends); messaging_limit_tier is the daily
// business-initiated-conversation cap (e.g. TIER_1K), which Meta lowers
// automatically when quality drops. Both explain a "failed" status that has
// nothing to do with template approval or the 24h window.
export async function getPhoneNumberQuality() {
  ...
  const res = await fetch(
    `https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}?fields=display_phone_number,verified_name,quality_rating,messaging_limit_tier`,
    { headers: { Authorization: `Bearer ${accessToken}` } }
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(data?.error?.message || `Could not fetch phone number status (${res.status})`);
  }
  return data;
}
...
export async function getMessageTemplateStatuses() {
  ...
  const res = await fetch(
    `https://graph.facebook.com/${GRAPH_VERSION}/${wabaId}/message_templates?fields=name,status,category&limit=100`,
    { headers: { Authorization: `Bearer ${accessToken}` } }
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
  ...
  // category matters as much as status: Meta can silently recategorize a
  // template during review regardless of what you submitted (see
  // submit*Template's own `category: 'UTILITY'`) — a template Meta decided
  // reads as MARKETING is subject to its own per-recipient engagement-based
  // send limits ("(#131049) ... to maintain healthy ecosystem engagement"),
  // completely independent of approval status or the sending number's own
  // quality rating. That's a real, silent failure mode worth surfacing.
  const statuses = {};
  for (const t of data.data || []) statuses[t.name] = { status: t.status, category: t.category };
  return statuses;
}
...
export async function getKnownTemplateStatuses() {
  ...
  return {
    ...
    clientAppLink: lookup('META_WHATSAPP_CLIENT_APP_TEMPLATE_NAME', 'client_app_link_v2'),
    dischargeFollowup: lookup('META_WHATSAPP_DISCHARGE_FOLLOWUP_TEMPLATE_NAME', 'discharge_followup_checkin_v2'),
