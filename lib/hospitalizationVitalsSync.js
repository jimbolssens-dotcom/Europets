...
// A hospitalization or day procedure booked straight from a consult
// (originating_visit_id — see POST /api/hospitalizations) already asked
// for weight/temperature in that consult's own Vitals & Exam section —
// carry that reading across the same way seedVitalsFromOrigin does for an
// admission's existing worksheet, so staff aren't asked to type in a
// reading that was already taken minutes earlier.
export async function seedVitalsFromVisit(newHospitalizationId, visitId) {
  try {
    const { data: visit } = await supabaseAdmin
      .from('visits')
      .select('weight_kg, temperature_c, attending_vet_id')
      .eq('id', visitId)
      .maybeSingle();
    if (!visit || (visit.weight_kg == null && visit.temperature_c == null)) return;

    const planItems = await vitalsPlanItemIds([newHospitalizationId]);
    const weightItemId = planItems.find((p) => p.kind === 'vitals_weight')?.id || null;
    const tempItemId = planItems.find((p) => p.kind === 'vitals_temperature')?.id || null;
    const planItemIds = [visit.weight_kg != null ? weightItemId : null, visit.temperature_c != null ? tempItemId : null].filter(Boolean);

    await supabaseAdmin.from('hospitalization_notes').insert([
      {
        hospitalization_id: newHospitalizationId,
        author_id: visit.attending_vet_id || null,
        note_date: new Date().toISOString().slice(0, 10),
        weight_kg: visit.weight_kg ?? null,
        temperature_c: visit.temperature_c ?? null,
        plan_item_ids: planItemIds,
        plan_item_id: planItemIds[0] || null,
        notes: 'Synced automatically from the consult.',
      },
    ]);
  } catch {
    // Best-effort — the new record still works fine starting blank.
