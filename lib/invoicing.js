...
// on the invoice yet (alreadyInvoicedIds) and is still marked billable —
// an item logged with billable=false (e.g. the owner already has this
// medication at home) stays on the treatment plan as a clinical record
// but never becomes a charge, see migration 096 — tagged with
...
function buildNewTreatmentItemLines(treatmentItems, invoiceId, clinicSettings, alreadyInvoicedIds) {
  return (treatmentItems || [])
    .filter((item) => item.goods_services && item.billable !== false && !alreadyInvoicedIds.has(item.id))
    .map((item) => {
  ...
  if (invoice.visit_id) {
    try {
      ...
      if (visit && visit.status !== 'complete') {
        const { data: completedVisit, error } = await supabase
          .from('visits')
