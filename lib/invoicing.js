...
export async function removeTreatmentItemFromInvoiceLines(supabase, treatmentItemId) {
  ...
  const { data: lines, error } = await supabase
    .from('invoice_line_items')
    .select('id, invoice_id, quantity, unit_price, description, administration_method, source_treatment_item_ids')
  ...
  for (const line of lines) {
    ...
    const newQuantity = Math.round((Number(line.quantity) - removedQuantity) * 100) / 100;

    if (remainingIds.length === 0 || newQuantity <= 0) {
    ...
    updated = applyAdministrationFee(updated, line.administration_method, clinicSettings, newQuantity);
