...
export async function resolveInvoiceHospitalizationFamily(supabase, hospitalizationId) {
  ...
  for (const member of members || []) {
    ...
    const date = member.admitted_at ? formatShortDate(member.admitted_at) : '';
...
function oneOffLineFromTreatmentItem(item, invoiceId, clinicSettings, sectionLabelById = {}) {
  ...
  return applyAdministrationFee(line, item.administration_method, clinicSettings, 1);
}
...
// for the first time on this invoice — times_given is the number of times
// it was already logged by the time this sync ran, not always 1, so a
// medication given for several days before the invoice was first opened
// still lands as one line with the right count from the start. quantity is
// the locked per-dose amount: every tap of a Day Treatment Plan item logs
// that plan item's fixed quantity, so items sharing a consolidation key are
// expected to already agree on it — but rather than trust that and use
// items[0] alone (silently wrong if they don't), quantity is the true
// average (total logged amount / times_given), which is exactly the locked
// per-dose value whenever dosing really was uniform, and still bills the
// correct total otherwise. No instructions carried over (they can vary
// dose to dose; staff can type a line-level note directly on the invoice
// if needed).
function newConsolidatedLine(items, invoiceId, clinicSettings, sectionLabelById = {}) {
  ...
  const timesGiven = items.length;
  const totalAmount = items.reduce((sum, item) => sum + (Number(item.quantity) || 1), 0);
  const quantity = Math.round((totalAmount / timesGiven) * 100) / 100;
  const unit_price = Number(catalogItem.base_price);
  const line = {
    invoice_id: invoiceId,
    goods_service_id: catalogItem.id,
    ...
    quantity,
    times_given: timesGiven,
    unit_price,
    line_total: Math.round(unit_price * quantity * timesGiven * 100) / 100,
    instructions: null,
    administration_method: administrationMethod,
  };
  return applyAdministrationFee(line, administrationMethod, clinicSettings, timesGiven);
}
...
export async function syncInvoiceTreatmentItems(supabase, invoiceId, treatmentItems, sectionLabelById = {}) {
  const [{ data: clinicSettings }, { data: existingLines, error: existingError }] = await Promise.all([
    ...
    supabase
      ...
      .select('id, quantity, times_given, unit_price, description, consolidation_key, source_treatment_item_ids')
  ...
  for (const [key, items] of groups) {
    ...
    const addedAmount = items.reduce((sum, item) => sum + (Number(item.quantity) || 1), 0);
    const existingTimesGiven = Number(existingLine.times_given) || 1;
    const existingAmount = Number(existingLine.quantity) * existingTimesGiven;
    const newTimesGiven = existingTimesGiven + items.length;
    const newQuantity = Math.round(((existingAmount + addedAmount) / newTimesGiven) * 100) / 100;
    const unit_price = Number(existingLine.unit_price);
    let line = {
      description: stripAdministrationFeeTag(existingLine.description),
      line_total: Math.round(unit_price * newQuantity * newTimesGiven * 100) / 100,
    };
    line = applyAdministrationFee(line, items[0].administration_method, clinicSettings, newTimesGiven);
    lineUpdates.push({
      id: existingLine.id,
      quantity: newQuantity,
      times_given: newTimesGiven,
...
export async function replaceHospitalizationRateCharges(supabase, hospitalizationId, invoiceId, newGoodsServiceId) {
  ...
  const { data: invoiceLineRows, error: lineRowsError } = await supabase
    .from('invoice_line_items')
    .select('id, quantity, times_given, source_treatment_item_ids')
    .eq('invoice_id', invoiceId)
  ...
  // quantity is per-dose/per-day and times_given is the day/dose count (see
  // migrations/129) — merging lines has to combine the true total amount
  // (quantity × times_given) of each, not just add their per-dose quantities
  // together, or a case with more than one existing rate line (the
  // double-charge bug this repoints) would come out billing the wrong total.
  const totalAmount = invoiceLineRows.reduce((sum, l) => sum + Number(l.quantity) * (Number(l.times_given) || 1), 0);
  const totalTimesGiven = invoiceLineRows.reduce((sum, l) => sum + (Number(l.times_given) || 1), 0);
  const newQuantity = Math.round((totalAmount / totalTimesGiven) * 100) / 100;
  ...
  const { error: keepError } = await supabaseAdmin
    ...
    .update({
      ...
      unit_price,
      quantity: newQuantity,
      times_given: totalTimesGiven,
      line_total: Math.round(unit_price * newQuantity * totalTimesGiven * 100) / 100,
...
export async function recomputeInvoiceTotals(supabase, invoiceId) {
  ...
  // The total just moved — an invoice sitting at status 'paid' from an
  // earlier, smaller total is now stale the instant a new charge lands on
  // it (a re-sync picking up worksheet items logged after it was paid, a
  // line added by hand, a discount changed, ...): amount_paid no longer
  // covers total, but nothing above touches status, so it kept reading
  // "paid" — invoices list included — while genuinely underpaid. Re-derive
  // it here, in the one place every caller that changes a total already
  // goes through, rather than requiring each of them to remember to call
  // this separately (several didn't). Best-effort: the totals themselves
  // are already saved by this point, so a hiccup here shouldn't fail the
  // caller — it'll just catch up on the next recompute.
  const { data: withPayments, error: paymentsError } = await recomputeInvoicePayments(supabase, invoiceId);
  if (paymentsError) {
    console.error('Failed to re-derive payment status after recomputing invoice totals', invoiceId, paymentsError);
    return { data };
  }
  return { data: withPayments || data };
}

// Call this BEFORE actually deleting a treatment_items row (its quantity is
// read here, so the row still needs to exist) — see
// app/api/treatment-items/[id] and app/api/diagnostics/[id]. Once a
// treatment_item has been synced onto an invoice (syncInvoiceTreatmentItems),
// deleting it from the clinical record used to leave the invoice line built
// from it completely untouched: staff deleting a mistakenly-ordered test or
// medication kept silently, permanently charging for it — found on a live
// case where a duplicated test order had since been deleted from the
// worksheet, but the invoice still billed for all three "copies" of it.
//
// A one-off line (a consult's own treatment plan item — see
// oneOffLineFromTreatmentItem) always has exactly one source id, so removing
// it always empties the line and it's deleted outright. A consolidated line
// (a hospitalization worksheet item logged across several days — see
// newConsolidatedLine) loses this item's own quantity and, if it had an
// administration fee, has that recalculated for the new count — mirroring
// the same math syncInvoiceTreatmentItems uses to grow a line, just run in
// reverse. Best-effort: the clinical-record deletion this runs ahead of is
// the primary action and shouldn't be blocked by an invoice-side hiccup —
// callers log the returned error rather than failing the delete on it.
export async function removeTreatmentItemFromInvoiceLines(supabase, treatmentItemId) {
  const { data: treatmentItem } = await supabase
    .from('treatment_items')
    .select('quantity')
    .eq('id', treatmentItemId)
    .maybeSingle();
  const removedQuantity = Number(treatmentItem?.quantity) || 1;

  const { data: lines, error } = await supabase
    .from('invoice_line_items')
    .select(
      'id, invoice_id, quantity, times_given, unit_price, description, administration_method, source_treatment_item_ids'
    )
    .contains('source_treatment_item_ids', [treatmentItemId]);
  if (error) return { error };
  if (!lines || lines.length === 0) return { affected: 0 };
  ...
  for (const line of lines) {
    ...
    const remainingIds = (line.source_treatment_item_ids || []).filter((id) => id !== treatmentItemId);
    const existingTimesGiven = Number(line.times_given) || 1;
    const existingAmount = Number(line.quantity) * existingTimesGiven;
    const newTimesGiven = existingTimesGiven - 1;
    const newAmount = Math.round((existingAmount - removedQuantity) * 100) / 100;

    if (remainingIds.length === 0 || newTimesGiven <= 0 || newAmount <= 0) {
      const { error: deleteError } = await supabaseAdmin.from('invoice_line_items').delete().eq('id', line.id);
      if (deleteError) return { error: deleteError };
      continue;
    }

    if (clinicSettings === null) {
      const { data: settings } = await supabase.from('clinic_settings').select('*').eq('id', true).maybeSingle();
      clinicSettings = settings || {};
    }
    const unit_price = Number(line.unit_price);
    const newQuantity = Math.round((newAmount / newTimesGiven) * 100) / 100;
    let updated = {
      description: stripAdministrationFeeTag(line.description),
      line_total: Math.round(unit_price * newQuantity * newTimesGiven * 100) / 100,
    };
    updated = applyAdministrationFee(updated, line.administration_method, clinicSettings, newTimesGiven);
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        ...
        source_treatment_item_ids: remainingIds,
      })
      .eq('id', line.id);
    if (updateError) return { error: updateError };
  }
