...
export const CHECKIN_CATEGORIES = [
  ...
  },
  {
    ...
    label: 'Medication',
    // A single toggle tile, not a scale like the categories above — there's
    // nothing useful to say beyond "yes, it was given" (a cleaner isn't
    // choosing which medication, that's still the vet's worksheet/catalog).
    options: [{ value: 'given', icon: '💊', label: 'Given' }],
  },
  {
    key: 'force_feeding_done',
    label: 'Force-Feeding',
    options: [{ value: 'done', icon: '🍼', label: 'Done' }],
  },
];
...
export function buildEmpathicCheckinText(note, patientName) {
  ...
  const care = [
    note?.medication_given === 'given' ? 'medication was given as scheduled' : null,
    note?.force_feeding_done === 'done'
      ? `the team helped ${name} along with some hand-feeding`
      : null,
  ].filter(Boolean);
  if (care.length > 0) {
    sentences.push(`${capitalize(joinWithAnd(care))}.`);
  }
