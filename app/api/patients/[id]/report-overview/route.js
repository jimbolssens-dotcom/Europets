...
export async function GET(request, { params }) {
  ...
  const [dental, surgical, ultrasound, xray, diagnostics] = await Promise.all([
    ...
    fetchByPatient('diagnostics', 'id, visit_id, hospitalization_id, created_at, type, result', visitIds, hospIds),
  ]);
  ...
  const rows = [
    ...
    ...diagnostics.map((r) => ({ ...r, source: r.visit_id ? 'consult' : 'hospitalization', kind: r.type || 'Diagnostic test', date: r.created_at, href: recordHref(r) })),
