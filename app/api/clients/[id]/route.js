...
export async function GET(request, { params }) {
  // Reachable without the staff PIN now (the client app reads its own
  // client record — see middleware.js) — a non-staff caller must be
  // asking for their own record. PATCH/DELETE below stay off the public
  // path list entirely, so they still require the staff PIN as before.
...
export async function DELETE(request, { params }) {
  ...
  if (attachments?.length) {
    await supabaseAdmin.storage.from('consult-files').remove(attachments.map((a) => a.file_path));
