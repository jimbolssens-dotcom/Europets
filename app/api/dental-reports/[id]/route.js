...
const EDITABLE_FIELDS = ['performed_by', 'findings', 'procedures_performed', 'notes', 'ai_summary'];
...
export async function GET(request, { params }) {
  ...
  // embed off dental_reports — see the report-pdf route in this same
  // folder for why.
  if (data.visit_id) {
...
export async function DELETE(request, { params }) {
  const { error } = await supabaseAdmin.from('dental_reports').delete().eq('id', params.id);
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
