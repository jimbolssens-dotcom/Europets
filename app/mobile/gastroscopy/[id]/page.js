// app/mobile/gastroscopy/[id]/page.js
// Record a gastroscopy report from a phone — same pattern as
...
export default function MobileGastroscopyReportPage() {
  const { id } = useParams();
  const router = useRouter();
  const [report, setReport] = useState(null);

  useEffect(() => {
    fetch(`/api/gastroscopy-reports/${id}`)
      .then((res) => res.json())
      .then(setReport);
  }, [id]);
    ...
    <div className="mobile-page">
      ...
      {report && (
        <>
          ...
          <AudioRecorder entityType="gastroscopy_report" entityId={id} />

          <h2 className="mobile-section-header">Photos</h2>
          <AttachmentSection entityType="gastroscopy_report" entityId={id} />
