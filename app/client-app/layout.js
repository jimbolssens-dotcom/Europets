...
//
// SECURITY NOTE: logging in requires a real WhatsApp-delivered one-time
// code (see page.js, lib/clientAppAuth.js, app/api/client-app/auth/*)
// backing a signed, httpOnly session cookie — not just "type a phone
// number on file". This route and every client-app-facing API route
// (/api/patients, /api/appointments, /api/clients/:id, etc.) are public
// (see middleware.js's PUBLIC_PATTERNS/CLIENT_APP_READ_PATTERNS) — each of
// those routes checks in-route (via lib/clientAppAuth.js's
// getClientSession) that the client_id/patient_id actually being asked
// for belongs to the caller's own session, or that the caller is staff.
// Adding a new client-app-facing route later means adding that same check
// to it — the middleware pattern alone doesn't do it.
//
