...
const labels = { blood: 'Blood test', ultrasound: 'Ultrasound', xray: 'X-ray', pcr: 'PCR', dental: 'Dental', surgical: 'Surgical' };
export default function HospitalizationTestReports({ hospitalizationId, notes = [] }) {
  ...
  const hasTest = notes.some((note) => note.treatment_items?.some((item) =>
    String(item.goods_services?.main_category || '').toLowerCase() === 'test' || /blood|cbc|ultrasound|x-ray|xray|radiograph|awalife|pcr|dental|surg/i.test(item.goods_services?.name || '')
  ));
