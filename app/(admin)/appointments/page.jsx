...
function AppointmentsPageInner() {
  ...
  useEffect(() => { function recalc() { const el = scheduleWrapRef.current; if (el) setPixelsPerMinute(recalcPixelsPerMinute(el.getBoundingClientRect().top)); } recalc(); window.addEventListener('resize', recalc); return () => window.removeEventListener('resize', recalc); }, [loading, rooms.length, vets.length, calendarView]);
  ...
  async function checkIn(appointmentId) { await fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ appointment_id: appointmentId }) }); setRefreshTick((n) => n + 1); }
