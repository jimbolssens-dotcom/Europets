...
function AppointmentsPageInner() {
  ...
  async function openConsult(appointment) { if (openingConsultId) return; setOpeningConsultId(appointment.id); try { if (appointment.status === 'booked') { const res = await fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ appointment_id: appointment.id }) }); const data = await res.json(); if (res.status === 409 && data.existingVisitId) { alert('This patient already has an open consult — opening it now.'); return router.push(`/consults/${data.existingVisitId}`); } if (res.ok && data.id) return router.push(`/consults/${data.id}`); } else if (appointment.status === 'checked_in' || appointment.status === 'complete') { const res = await fetch(`/api/visits?appointment_id=${appointment.id}`); const data = await res.json(); const visit = Array.isArray(data) ? data[0] : null; if (visit?.id) return router.push(`/consults/${visit.id}`); } } finally { setOpeningConsultId(null); } }
