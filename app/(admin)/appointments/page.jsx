...
function AppointmentsPageInner() {
  ...
  const dayAppointments = useMemo(() => appointments.filter((a) => a.status !== 'cancelled' && a.status !== 'no_show' && toISODate(new Date(a.start_time)) === selectedDate), [appointments, selectedDate]);
  const weekAppointmentsByDate = useMemo(() => { const result = {}; for (const d of currentWeek) result[toISODate(d)] = []; for (const a of appointments) { if (a.status === 'cancelled' || a.status === 'no_show') continue; const iso = toISODate(new Date(a.start_time)); if (result[iso]) result[iso].push(a); } return result; }, [appointments, currentWeek]);
