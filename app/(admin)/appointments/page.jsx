...
function AppointmentsPageInner() {
  ...
  // Every currently-open (in_progress) consult, patient_id -> visit —
  // independent of how it was started (this appointment's own Checkin
  // button, a walk-in, or straight from the patient file). A booked
  // appointment whose patient shows up here is really already checked
  // in, even though appointments.status itself only gets flipped by this
  // page's own Checkin flow — see openVisitByPatient below.
  ...
  const [roomConflictBlock, setRoomConflictBlock] = useState(null);
  ...
  const selectedDateLabel = selectedDateObj.toLocaleDateString('en-GB', { weekday: 'long', day: '2-digit', month: '2-digit' });
  const weekLabel = `${currentWeek[0].toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' })} – ${currentWeek[6].toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}`;
  ...
  // Which patients currently have an open consult, regardless of which
  // screen started it — lets the list below show an appointment as
  // checked-in (and link straight into that consult) even when it was
  // opened some other way and appointments.status never got flipped.
  useEffect(() => {
    fetch('/api/visits?status=in_progress')
      .then((res) => res.json())
      .then((data) => setOpenVisits(Array.isArray(data) ? data : []));
  }, [refreshTick]);
  const openVisitByPatient = useMemo(() => {
    const map = new Map();
    for (const v of openVisits) if (v.patient_id) map.set(v.patient_id, v);
    return map;
  }, [openVisits]);
  ...
  useEffect(() => {
    ...
    ]).then(([roomsData, vetsData, staffData]) => {
    ...
    const channel = supabase.channel('appointments-view-changes')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'visits' }, () => setRefreshTick((n) => n + 1))
  ...
  // still theirs to read on the calendar) but drop out of the picker for a
  // brand-new booking.
  const bookableVets = useMemo(() => vets.filter((v) => v.active !== false), [vets]);
  // Any active staff member, not just vets — who a Staff Meeting/Other
  // booking is with.
  const bookableStaff = useMemo(() => staffList.filter((s) => s.active !== false), [staffList]);
  // The dedicated room every Staff Meeting/Other booking auto-lands in
  // (seeded by migration 109) — clinical bookings never offer it as a
  // choice, but it still gets its own column on the day view like any
  // other room, so a meeting is visible on the schedule.
  ...
  const countsByDate = useMemo(() => { const counts = {}; for (const a of appointments) { if (a.status === 'cancelled' || a.status === 'no_show') continue; const d = toISODate(new Date(a.start_time)); counts[d] = (counts[d] || 0) + 1; } return counts; }, [appointments]);
  // Cancelled/no-show appointments stay visible here (grayed out in
  // appointmentBlock/the list table below) rather than disappearing —
  // the slot was booked and that's still useful history — only the
  // month-view badge counts above exclude them, since those represent
  // how busy a day actually is.
  const dayAppointments = useMemo(() => appointments.filter((a) => toISODate(new Date(a.start_time)) === selectedDate), [appointments, selectedDate]);
  const weekAppointmentsByDate = useMemo(() => { const result = {}; for (const d of currentWeek) result[toISODate(d)] = []; for (const a of appointments) { const iso = toISODate(new Date(a.start_time)); if (result[iso]) result[iso].push(a); } return result; }, [appointments, currentWeek]);
  const liveDayAppointments = useMemo(() => { if (!dragMove && !dragResize) return dayAppointments; return dayAppointments.map((a) => { if (dragMove?.appointmentId === a.id) return { ...a, room_id: dragMove.roomId, start_time: startTimeFromMinutes(dragMove.dateISO, dragMove.startMinutes) }; if (dragResize?.appointmentId === a.id) return { ...a, duration_minutes: dragResize.duration }; return a; }); }, [dayAppointments, dragMove, dragResize]);
  // Cross-day dragging in week view moves an appointment out of its
  // origin day column and previews it in whichever column the cursor is
  // over — same idea as liveDayAppointments, just across the day axis
  // instead of the room axis.
  const liveWeekAppointmentsByDate = useMemo(() => {
    if (!dragMove || calendarView !== 'week') return weekAppointmentsByDate;
    const result = {};
    for (const [iso, apps] of Object.entries(weekAppointmentsByDate)) result[iso] = apps.filter((a) => a.id !== dragMove.appointmentId);
    const original = appointments.find((a) => a.id === dragMove.appointmentId);
    if (original && result[dragMove.dateISO]) {
      result[dragMove.dateISO] = [...result[dragMove.dateISO], { ...original, room_id: dragMove.roomId, start_time: startTimeFromMinutes(dragMove.dateISO, dragMove.startMinutes) }];
    }
    return result;
  }, [weekAppointmentsByDate, dragMove, calendarView, appointments]);
  ...
  function applySlotSelection(roomId, startMinutes, durationMinutes, dateISO = selectedDate) {
    ...
    if (room?.type === 'staff') { const rounded = Math.max(SNAP_MINUTES, Math.round(Math.max(durationMinutes, SNAP_MINUTES) / SNAP_MINUTES) * SNAP_MINUTES); setForm((f) => ({ ...f, time, room_id: roomId, type: 'meeting', duration_minutes: String(rounded) })); }
    else if (durationMinutes <= SNAP_MINUTES) { const type = room?.type === 'surgery' ? 'surgery' : 'consult'; setForm((f) => ({ ...f, time, room_id: roomId || f.room_id, type, duration_minutes: '10' })); }
  ...
  function startMoveAppointment(e, appointment) {
    if (e.button !== 0 || appointment.status === 'cancelled' || appointment.status === 'complete' || appointment.status === 'no_show') return;
    e.stopPropagation(); const trackEl = e.currentTarget.closest('.schedule-room-track'); if (!trackEl) return;
    const originalRoomId = appointment.room_id; const originalDateISO = toISODate(new Date(appointment.start_time)); const originalStartMinutes = minutesSinceOpen(appointment.start_time); const duration = appointment.duration_minutes; const totalMinutes = (CLOSE_HOUR - OPEN_HOUR) * 60; const grabOffsetMinutes = (e.clientY - trackEl.getBoundingClientRect().top) / pixelsPerMinute - originalStartMinutes; const startX = e.clientX; const startY = e.clientY; let moved = false;
    setDragMove({ appointmentId: appointment.id, roomId: originalRoomId, dateISO: originalDateISO, startMinutes: originalStartMinutes }); setHoverSlot(null);
    // Day view drops onto another ROOM column (same day, per-room tracks);
    // week view drops onto another DAY column (same room — there's no room
    // column to drop into there, see renderWeekView's data-date tracks).
    function resolve(clientX, clientY) {
      ...
      const startMinutes = Math.max(0, Math.min(snapped, totalMinutes - duration));
      if (calendarView === 'week') return { roomId: originalRoomId, dateISO: el?.dataset.date || originalDateISO, startMinutes };
      return { roomId: el?.dataset.roomId || originalRoomId, dateISO: originalDateISO, startMinutes };
    }
  ...
  async function patchAppointment(appointmentId, body) { const res = await fetch(`/api/appointments/${appointmentId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const data = await res.json().catch(() => null); if (!res.ok) { if (data?.code === 'not_on_roster') { setRosterBlock({ message: `${data.vet_name} isn't on the staff roster for that ${data.shift} (${data.date}).`, vetId: data.vet_id, vetName: data.vet_name, date: data.date, shift: data.shift, payload: { __reschedule: true, appointmentId, body } }); return { ok: false, rosterBlocked: true }; } if (data?.code === 'room_conflict') { setRoomConflictBlock({ appointmentId, message: data.error, retry: data.retry }); return { ok: false, roomConflict: true }; } const message = data?.error || 'Failed to update the appointment'; setScheduleError(message); return { ok: false, rosterBlocked: false, error: message }; } setScheduleError(null); setRosterBlock(null); setRoomConflictBlock(null); setRefreshTick((n) => n + 1); return { ok: true }; }
  async function resolveRoomConflict(newRoomId) { if (!roomConflictBlock || !newRoomId) return; const { appointmentId, retry } = roomConflictBlock; await patchAppointment(appointmentId, { ...retry, room_id: newRoomId }); }
  ...
  async function handleEditSave(body) { const result = await patchAppointment(editingAppointment.id, body); if (result.ok || result.rosterBlocked || result.roomConflict) { setEditingAppointment(null); return {}; } return { error: result.error }; }
  ...
  async function handleSubmit(e) { e.preventDefault(); if (!form.time || (!form.room_id && form.type !== 'video')) { setError(form.type === 'video' ? 'Pick a date/time on the schedule' : 'Pick a date/time on the schedule and select a room'); return; } if (form.type !== 'meeting' && (!form.client_id || !form.patient_id)) { setError('Select an owner and patient'); return; } setSubmitting(true); setError(null); setRosterBlock(null); const startTime = new Date(`${selectedDate}T${form.time}:00`); await submitAppointment({ patient_id: form.type === 'meeting' ? null : form.patient_id, room_id: form.room_id || null, vet_id: form.vet_id || null, type: form.type, start_time: startTime.toISOString(), duration_minutes: form.type === 'surgery' || form.type === 'meeting' ? Number(form.duration_minutes) : undefined, reason: form.reason, date: selectedDate, shift: startTime.getHours() < 12 ? 'morning' : 'afternoon' }); setSubmitting(false); }
  ...
  async function deleteAppointment(id) { if (!confirm('Delete this cancelled appointment? This cannot be undone.')) return; await fetch(`/api/appointments/${id}`, { method: 'DELETE' }); setRefreshTick((n) => n + 1); }
  // A still-'booked' appointment whose patient already has an open
  // consult (started some other way — the patient file directly, a
  // walk-in, ...) is really already checked in; reflect that here so the
  // list and its Checkin/View Consult button match reality instead of
  // the appointment's own status field, which only this page's Checkin
  // flow ever sets.
  function appointmentOpenVisit(a) {
    if (a.status !== 'booked' || a.type === 'meeting' || a.type === 'surgery' || !a.patient_id) return null;
    return openVisitByPatient.get(a.patient_id) || null;
  }
  function appointmentRowStatus(a) { return appointmentOpenVisit(a) ? 'checked_in' : a.status; }
  function reminderMessage(a) { const dateLabel = new Date(a.start_time).toLocaleDateString('en-GB', { weekday: 'long', day: '2-digit', month: '2-digit' }); return `Hi ${a.clients?.full_name || 'there'}, this is a reminder that ${a.patients?.name || 'your pet'} has an appointment at Europets Clinic on ${dateLabel} at ${formatTime(a.start_time)}. See you then! — Europets Clinic`; }
  ...
  async function openConsult(appointment) {
    if (openingConsultId) return; setOpeningConsultId(appointment.id);
    try {
      // The patient may already have an open consult that wasn't started
      // from this appointment at all (opened from the patient file, a
      // walk-in, ...) — openVisitByPatient catches that regardless of
      // what appointments.status says, so jump straight in rather than
      // going through the booked/checked_in branches below.
      const knownOpenVisit = appointment.patient_id ? openVisitByPatient.get(appointment.patient_id) : null;
      if (knownOpenVisit) return router.push(`/consults/${knownOpenVisit.id}`);
      if (appointment.status === 'booked') {
        const res = await fetch('/api/visits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ appointment_id: appointment.id }) });
        const data = await res.json();
        if (res.status === 409 && data.existingVisitId) { alert('This patient already has an open consult — opening it now.'); return router.push(`/consults/${data.existingVisitId}`); }
        ...
        const data = await res.json();
        let visit = Array.isArray(data) ? data[0] : null;
        // Falls back to the patient's open consult even when this
        // appointment isn't the one it's linked to (or isn't linked to
        // any appointment at all) — same reasoning as the knownOpenVisit
        // check above, kept here too in case openVisits hasn't loaded yet.
        if (!visit?.id && appointment.patient_id) {
          const fallback = await fetch(`/api/visits?patient_id=${appointment.patient_id}&status=in_progress`).then((r) => r.json());
          visit = Array.isArray(fallback) ? fallback[0] : null;
        }
        if (visit?.id) return router.push(`/consults/${visit.id}`);
      }
    } finally { setOpeningConsultId(null); }
  }
  ...
  function openAppointmentRecord(appointment) { if (appointment.type === 'meeting') return; return appointment.type === 'surgery' ? openDayProcedure(appointment) : openConsult(appointment); }
  ...
  return <div className="appointments-page">
    ...
    <div className={`schedule-layout ${calendarView === 'day' ? 'day-view' : ''}`}><div className="schedule-left-col"><AppointmentRequestsPanel rooms={rooms} vets={vets} onApproved={() => setRefreshTick((n) => n + 1)} /></div><div className="schedule-main">{scheduleError && <p className="error">{scheduleError}</p>}{vets.length > 0 && <div className="vet-legend">{vets.map((v) => <span key={v.id} className="vet-legend-item"><span className="vet-legend-swatch" style={{ background: colorForVetAppt(v.id, 'consult').bg, borderColor: colorForVetAppt(v.id, 'consult').fg }} title="Consult" /><span className="vet-legend-swatch" style={{ background: colorForVetAppt(v.id, 'surgery').bg, borderColor: colorForVetAppt(v.id, 'surgery').fg }} title="Surgery / Day Procedure" />{v.full_name}</span>)}<span className="vet-legend-item"><span className="vet-legend-swatch" style={{ background: UNASSIGNED_STAFF_COLOR.bg, borderColor: UNASSIGNED_STAFF_COLOR.fg }} />Unassigned</span><span className="vet-legend-hint">light = Consult · dark = Surgery/Day Procedure</span></div>}{loading ? <p>Loading...</p> : rooms.length === 0 ? <p>No rooms set up yet — add one on the <a href="/rooms">Rooms</a> page first.</p> : calendarView === 'week' ? renderWeekView() : renderDayView()}</div><div className="booking-panel"><form className="card" ref={bookingFormRef} onSubmit={handleSubmit}><h2>Book Appointment</h2>{error && <p className="error">{error}</p>}<p>{form.time ? `Booking ${selectedDateLabel} at ${form.time}${form.room_id ? ` in ${rooms.find((r) => r.id === form.room_id)?.name || ''}` : form.type === 'video' ? '' : ' — select a room below'}` : 'Click a time on the schedule to start a booking'}</p>{form.type !== 'meeting' && (selectedOwner ? <p className="booking-owner-picked">Owner: <strong>{selectedOwner.full_name}</strong>{' '}<button type="button" onClick={() => { setSelectedOwner(null); setForm((f) => ({ ...f, client_id: '', patient_id: '' })); }}>Change</button></p> : <ClientOrPatientSearch placeholder="Search clients or patients..." onPickClient={(c) => { setSelectedOwner({ id: c.id, full_name: c.full_name }); setForm((f) => ({ ...f, client_id: c.id, patient_id: '' })); }} onPickPatient={(p) => { setSelectedOwner({ id: p.client_id, full_name: p.clients?.full_name || '' }); setForm((f) => ({ ...f, client_id: p.client_id, patient_id: p.id })); }} />)}{form.type !== 'meeting' && <SearchSelect items={clientPatients} value={form.patient_id} onChange={(patient_id) => setForm((f) => ({ ...f, patient_id }))} getLabel={(p) => p.name} getSubLabel={(p) => p.species} placeholder="Select patient..." disabled={!form.client_id} />}<select value={form.type} onChange={(e) => { const nextType = e.target.value; setForm((f) => ({ ...f, type: nextType, duration_minutes: nextType === 'meeting' ? '30' : '10', room_id: nextType === 'meeting' ? (staffRoom?.id || '') : '' })); }}><option value="consult">Consult (15 min)</option><option value="video">Video Consult (15 min)</option><option value="surgery">Surgery (10-min increments)</option><option value="meeting">Staff Meeting / Other</option></select>{(form.type === 'surgery' || form.type === 'meeting') && <input type="number" min={form.type === 'meeting' ? 5 : 10} step={form.type === 'meeting' ? 5 : 10} value={form.duration_minutes} onChange={(e) => setForm((f) => ({ ...f, duration_minutes: e.target.value }))} placeholder="Duration (minutes)" />}{form.type === 'meeting' ? <p className="visit-meta">Room: {staffRoom?.name || 'Staff Room'} (automatic)</p> : form.type === 'video' ? <p className="visit-meta">🎥 No room needed — video consult</p> : <select required value={form.room_id} onChange={(e) => setForm((f) => ({ ...f, room_id: e.target.value }))}><option value="">Select room...</option>{clinicalRooms.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}</select>}<select value={form.vet_id} onChange={(e) => { setForm((f) => ({ ...f, vet_id: e.target.value })); setRosterBlock(null); }}><option value="">{form.type === 'meeting' ? 'Select staff member (optional)...' : 'Select vet (optional)...'}</option>{(form.type === 'meeting' ? bookableStaff : bookableVets).map((v) => <option key={v.id} value={v.id}>{v.full_name}</option>)}</select><input placeholder={form.type === 'meeting' ? 'Meeting note / details' : 'Reason for visit'} value={form.reason} onChange={(e) => setForm((f) => ({ ...f, reason: e.target.value }))} /><button type="submit" disabled={submitting || !form.time || (!form.room_id && form.type !== 'video')}>{submitting ? 'Booking...' : 'Book'}</button></form></div></div>
    <h2>{selectedDateLabel} — list</h2><div className="appointments-day-list-wrap"><table><thead><tr><th>Time</th><th>Type</th><th>Patient</th><th>Client #</th><th>Reason</th><th>Room</th><th>Vet</th><th>Status</th><th></th></tr></thead><tbody>{dayAppointments.length === 0 && <tr><td colSpan={9}>No appointments booked for this day.</td></tr>}{dayAppointments.map((a) => <tr key={a.id} className={a.status === 'cancelled' || a.status === 'no_show' ? 'appointments-row-inactive' : ''}><td>{formatTime(a.start_time)} ({a.duration_minutes}m)</td><td>{a.type}</td><td>{a.type === 'meeting' ? '—' : (a.patients?.name || (a.patient_id ? '' : '(unlinked)'))}</td><td>{a.clients?.client_number ? <a href={`/clients/${a.client_id}`} onClick={(e) => e.stopPropagation()}>{a.clients.full_name} (#{a.clients.client_number})</a> : '—'}</td><td>{a.reason}</td><td>{a.rooms?.name}</td><td>{a.staff?.full_name || '—'}</td><td>{appointmentRowStatus(a)}{appointmentOpenVisit(a) && <span className="visit-meta"> (checked in elsewhere)</span>}</td><td>{a.status === 'booked' && a.patient_id && !appointmentOpenVisit(a) && <button type="button" disabled={openingConsultId === a.id} onClick={() => openAppointmentRecord(a)}>{openingConsultId === a.id ? 'Checking in…' : 'Checkin'}</button>}{(a.status === 'checked_in' || a.status === 'complete' || appointmentOpenVisit(a)) && <button type="button" disabled={openingConsultId === a.id} onClick={() => openAppointmentRecord(a)}>{openingConsultId === a.id ? 'Opening…' : a.type === 'surgery' ? 'View Day Procedure' : a.type === 'video' ? '🎥 View Video Consult' : 'View Consult'}</button>}{a.status === 'booked' && !appointmentOpenVisit(a) && a.clients?.phone && <button type="button" onClick={() => sendReminder(a)}>💬 Remind</button>}{a.status !== 'cancelled' && a.status !== 'complete' && a.status !== 'no_show' && <button type="button" onClick={() => cancelAppointment(a.id)}>Cancel</button>}{(a.status === 'cancelled' || a.status === 'no_show') && <button type="button" onClick={() => deleteAppointment(a.id)}>Delete</button>}{a.reminder_sent_at && <span className="visit-meta"> Reminded {formatTime(a.reminder_sent_at)}</span>}</td></tr>)}</tbody></table></div>
    ...
    {roomConflictBlock && <div className="roster-block-backdrop"><div className="roster-block-modal" role="alertdialog" aria-live="assertive"><div className="roster-block-icon">🚪</div><p className="roster-block-message">{roomConflictBlock.message}</p><select value={pickedRoomId} onChange={(e) => setPickedRoomId(e.target.value)}><option value="">Choose a free room...</option>{rooms.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}</select><div className="roster-block-actions"><button type="button" onClick={() => resolveRoomConflict(pickedRoomId)} disabled={!pickedRoomId}>✅ Move to this room</button><button type="button" className="secondary" onClick={() => setRoomConflictBlock(null)}>✖️ Cancel</button></div></div></div>}
    {editingAppointment && <EditAppointmentModal appointment={editingAppointment} rooms={rooms} vets={vets} staffList={bookableStaff} onClose={() => setEditingAppointment(null)} onSave={handleEditSave} />}
  </div>;
}
