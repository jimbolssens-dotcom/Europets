...
function AppointmentsPageInner() {
  ...
  async function handleSubmit(e) { e.preventDefault(); if (!form.time || !form.room_id) { setError('Pick a date/time on the schedule and select a room'); return; } if (form.type !== 'meeting' && (!form.client_id || !form.patient_id)) { setError('Select an owner and patient'); return; } setSubmitting(true); setError(null); setRosterBlock(null); const startTime = new Date(`${selectedDate}T${form.time}:00`); await submitAppointment({ patient_id: form.type === 'meeting' ? null : form.patient_id, room_id: form.room_id, vet_id: form.vet_id || null, type: form.type, start_time: startTime.toISOString(), duration_minutes: form.type === 'surgery' || form.type === 'meeting' ? Number(form.duration_minutes) : undefined, reason: form.reason, date: selectedDate, shift: startTime.getHours() < 12 ? 'morning' : 'afternoon' }); setSubmitting(false); }
