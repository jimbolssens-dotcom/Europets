// app/add/page.jsx
// The Add tab — a normal page like every other nav tab (Invite,
// Appointments, ...), not a dropdown over whatever page you were on.
// Add Client and Add Patient side by side, same fields and behavior as
// the full forms on the Clients/Patients pages (multi-phone editor,
// Emirates ID scan, duplicate detection for the client; owner/species/
// breed/color/... for the patient) — those pages are unchanged and still
// have their own copies for when a vet is already there. The one
// deliberate difference from the old standalone pages: adding a client
// here doesn't navigate away — instead it pre-fills the Add Patient
// column's owner with the client just created, so that first pet can
// still be added in one flow without leaving this page. Arriving here
// with ?client_id=X (e.g. "Add a patient for this client" on a client's
// detail page) pre-fills that same owner slot.
...
function AddPageContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/clients/${prefilledClientId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data && !data.error) setOwner(data);
      });
  ...
  async function createClient() {
    ...
    // without leaving this page or retyping the owner search.
    setOwner(data);
  }
  ...
  return (
    <div>
      ...
      <div className="two-col">
        <form className="card" onSubmit={handleClientSubmit}>
          <h2>Add Client</h2>
